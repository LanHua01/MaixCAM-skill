...
首次使用时，若题面或附件没有提供足够信息，Skill 会先确认题型与评分、识别对象和输出、MaixCAM/相机、场地限制，以及是否实际存在通信、执行机构、独立 UI、现场调参和模型需求；只有存在的模块才继续追问屏幕、引脚、协议、急停或数据集细节。
...
- **条件在线检索**：陌生题或需核验规则/API/版本时，先查本地题目索引，再查官方资料与公开案例；输出链接、事实、经验和待确认项，用户可明确禁止联网。

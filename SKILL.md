...
用户确认方案后，默认生成完整、可打开的 MaixVision 工程目录，而不是只输出零散代码片段。目录必须有唯一 `main.py` 入口、被入口实际导入的模块、所需配置、测试脚本以及已提供的模型/资源；先展示目录树，再生成所有文件内容。
...
生成的 MaixPy 工程代码注释默认使用中文；保留 Python 关键字、MaixPy API、文件名、标识符和协议字段的原有英文拼写。注释只解释模块职责、硬件假设、单位/坐标、安全状态、非直观时序和调参含义，避免逐行重复代码含义。用户明确要求其他语言时按用户要求执行。

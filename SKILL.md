...
本 skill 用于 MaixCAM Pro + MaixPy 电赛视觉工程的题面拆解、得分路线、方案选择、实现、联调、现场调参和验收。它固化工程方法与 MaixPy 约束，不绑定某一年赛题、固定串口协议、坐标单位、心跳、任务模式或模型。

## 硬规则

1. 所有正向代码示例必须符合 MaixPy 官方 API；不得混入 OpenMV、K210、树莓派或 PC OpenCV 专属写法。
2. 先读取当前题面与评分，先让基础得分的相机、视觉、通信、执行机构各自可测并形成最小闭环，再做提高或发挥部分。
3. 协议、坐标、频率、ACK、心跳、UI、模型和任务模式均为待确认决策，不得擅自固定。
4. 题目检索固定为：当前题面与评分 → `references/vision-archetypes.md` 视觉原型 → `references/verified-vision-topics.md` 已核验历史题 → 回到当前题面确认。`references/topic-title-archive.md` 只能提供公开检索键；未经题面核验的题名不得影响方案。
5. 用户提供已有工程、接线、协议或下位机代码时，先阅读并提取已确认决策；优先局部修复和最小验证，不平行重写。只有改动影响识别路线、协议、引脚、坐标、安全或验收时，才回到方案选择门禁。
6. 同一故障连续两次修改仍未解决时，停止补丁式修改，转入源码到板端的全链路一致性审查。
...
用户给出已有工程、板端异常、部署漂移或“修改后仍失败”时，与快速验证、新项目并列进入故障接管，不重新走完整新项目问询。读取 `references/existing-project-integration.md`，固定执行：

```text
确认板端实际版本 → 阅读入口与调用链 → 最小复现 → 分层定位
→ 根因与证据 → 一次只修一个根因 → 板端复测
```

- 先记录源码快照、入口、资源、接口契约、部署方式、板端身份、失败次数和最后通过基线；保留已确认的引脚、协议、坐标和安全联锁。
- 每轮只修一个有证据的根因，并输出状态卡：冻结接口、本轮范围、最后板端通过版本、当前问题、修改结果、回退点和待实测项。
- 同一故障两次修改失败后，核对源码、工作区、入口、配置、资源哈希、部署包、板端身份和日志；完成审查前不提出第三个猜测性补丁。
- `scripts/` 中的工具只做静态检查和离线证据比较，不会自动读取设备；板端现象仍以用户日志和目标板复测为准。

## 条件自动检索与来源分级

当题目陌生、题面规则/硬件/API/版本需要核验、需要外部案例，或候选路线存在明显不确定性时，先按视觉原型和已核验题检索；仍需题源线索时再查题名档案，然后按 `references/research-guidance.md` 在线检索。用户明确“禁止联网”时跳过在线检索；浏览能力不可用时，给出可复用的检索关键词和待确认假设，不得伪造来源或检索结论。
...
- 核心配置：题型/评分、识别目标与输出、精度/时限/失效动作、MaixCAM Pro/相机/安装方式、场地光照与目标条件、允许/禁止的硬件或算法，以及比赛剩余时间。剩余时间未知时询问，不虚构绝对工期。
...
新项目使用 `templates/scoring-roadmap.md` 将每条题面要求映射到分值层级、验收动作、模块、依赖、风险、得分收益、失败影响和回退点。主线固定为：

```text
读取当前题面与评分 → 视觉原型匹配 → 已核验历史题参考 → 回到当前题面确认
→ 基础得分最小闭环 → 提高/发挥部分 → 现场调参与降级 → 接口冻结与稳定性验收
```

- 基础得分闭环未在目标板独立通过，不进入提高或发挥；未实测只能标为“待实测”。
- 新功能必须映射到明确评分项，并有关闭开关或回退点；造成基础项回归时立即回退。
- 用户未给比赛剩余时间时先询问；默认只建议预留最后 25% 时间用于场地、连续运行、备份和修复，具体比例与冻结时刻由用户确认。
- 进入稳定性预留窗口后冻结引脚、协议、坐标、模型和模块接口，只允许参数调整或有证据的明确修复。
...
- 先读取 `references/maixpy-vision-capabilities.md`，按 Blob/LAB、Line/Edge、Geometry、QR/AprilTag、Detection、OBB、Segmentation、Tracking 的版本、性能风险和降级路线筛选；官方未注明最低版本时要求目标板核验。
- 颜色/激光：优先 `img.find_blobs()`、ROI、曝光/白平衡和时序过滤。
- 黑线、边框、靶纸和尺寸：优先 Line/Edge、几何、标定、单应性或物理坐标映射。
...
- 方向、不规则像素区域或跨帧身份：只有它们属于真实评分需求时才分别比较 OBB、Segmentation 或 Tracking；普通框加几何、传统轮廓或短时缓存足够时不升级。
- 混合方案：传统视觉提供几何/ROI 约束，模型输出类别或复杂目标。

需要按电赛视觉模式细化识别、标定、状态和安全策略时，读取 `references/vision-archetypes.md` 中与题面匹配的章节；不要把不相干题型的做法塞进当前工程。
...
- 已有工程的调试、增量功能或下位机联调按“已有工程故障接管模式”执行；保留已验证的端口、引脚、波特率、协议和安全联锁，只追问本次改动所需的信息。
...
修改 Skill、模板或参考资料后，维护者必须使用 `references/validation-scenarios.md` 执行单轮断言，并用 `references/long-run-validation-scenarios.md` 验证多轮状态保持。保存的回答可通过 `scripts/validate_skill_scenarios.py` 离线检查关键词、禁止项和状态结构；语义正确性仍由独立新会话人工判断。任何“信息不足时直接生成最终协议/工程”“已有工程却未先阅读”“两次失败后继续猜测性补丁”“YOLO 无数据却声称可训练完成”的失败，都阻止发布。
